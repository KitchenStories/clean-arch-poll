...
def add_question(question: adapters.QuestionAdapter) -> adapters.QuestionAdapter:
    uc = use_cases.PollAddUseCase(repo_instances.mem_poll_repo)
...
def get_poll(question_id: UUID) -> adapters.QuestionAdapter:
    uc = use_cases.PollGetUseCase(repo_instances.mem_poll_repo)
...
@router.post('/{vote_id}/vote', response_model=adapters.ChoiceAdapter)
def vote_choice(vote_id: UUID):
    uc = use_cases.PollVoteUseCase(repo_instances.mem_choice_repo)
